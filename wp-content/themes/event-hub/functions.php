<?php
/**
 * Event Hub functions and definitions
 *
...
function event_hub_setup() {
	...
	register_nav_menus( array(
		'menu-1' => esc_html__( 'Primary', 'event-hub' ),
	) );
...
function event_hub_scripts() {
	wp_enqueue_style( 'event-hub-style', get_stylesheet_uri() );

	wp_enqueue_script( 'event-hub-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'event-hub-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'event_hub_scripts' );
