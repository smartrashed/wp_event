...
function event_term_comment_template( $comment, $args, $depth ) {
    ...
    switch ( $comment->comment_type ) :
        ...
        case 'trackback' :
            ...
            <p><?php esc_html__( 'Pingback:', 'event-term' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( esc_html__( '(Edit)', 'event-term' ), '<span class="edit-link">', '</span>' ); ?></p>
            <?php
            break;
        default :
                ...
                <div class="contents">
                    <div class="comment-meta">
                        ...
                        <a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                            <time datetime="<?php echo get_comment_time( 'c' ); ?>">
                                <?php echo sprintf( esc_html__( '%1$s at %2$s', 'event-term' ), get_comment_date(), get_comment_time() ) ?>
                            </time>
                        </a>
                    </div><!-- /.comment-meta -->
                    ...
                    <div class="content">
                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="comment-awaiting-moderation"><?php esc_html__( 'Your comment is awaiting moderation.', 'event-term' ); ?></p>
                        <?php endif; ?>

                        <?php comment_text(); ?>
                        <?php edit_comment_link( esc_html__( 'Edit', 'event-term' ), '<p class="edit-link">', '</p>' ); ?>
                    </div><!-- /.content -->

                    <?php comment_reply_link( array_merge( $args, array( 'reply_text' => esc_html__( 'Reply', 'event-term' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
                </div><!-- /.contents -->
