 ...
 */
function event_term_scripts() {
    $gmap_api = 'AIzaSyBP3wA0iYoevRqn8LmJglqYXhi_r7Xt33Y';
    if (function_exists('cs_get_option')):
        $gmap_api = cs_get_option('et_google_api');
    endif;    
    wp_enqueue_style('event-term-stylesheet', get_stylesheet_uri());
    wp_enqueue_style('bootstrap', EVENTTERM_ASSETS . '/css/bootstrap.min.css', false);
    wp_enqueue_style('font-awesome', EVENTTERM_ASSETS . '/css/font-awesome.min.css', false);
    wp_enqueue_style('owl-carousel', EVENTTERM_ASSETS . '/css/owl.carousel.css', false);
    wp_enqueue_style('hover-min', EVENTTERM_ASSETS . '/css/hover-min.css', false);
    wp_enqueue_style('event-term-style', EVENTTERM_ASSETS . '/css/style.css', false);
    wp_enqueue_style('event-term-responsive', EVENTTERM_ASSETS . '/css/responsive.css', false);       

    wp_enqueue_script('google-maps', 'https://maps.googleapis.com/maps/api/js?key='.$gmap_api, null, null, true);
    wp_enqueue_script('event-term-plugins', EVENTTERM_ASSETS . '/js/plugins.js', array('jquery'), false, true);
    wp_enqueue_script('event-term-functions', EVENTTERM_ASSETS . '/js/functions.js', array('jquery'), '1.0.0', true);
    wp_enqueue_script('mediaelement');
    if (is_singular() && comments_open() && get_option('thread_comments')){
        wp_enqueue_script('comment-reply');
    }
}
add_action( 'wp_enqueue_scripts', 'event_term_scripts', 90);

if ( ! function_exists( 'event_term_fonts_url' ) ):
    function event_term_fonts_url(){
         ...
         */
        if ('off' !== _x('on', 'Google font: on or off', 'event-term')){
            $font_families = array();
...
if ( ! function_exists( 'event_term_custom_styles' ) ):
function event_term_custom_styles() {
    $et_font = cs_get_option('et_font_family');   
    $et_font_family = $et_font['family'];   
...
if ( ! function_exists( 'event_term_enqueue_gmap' ) ):
    function event_term_enqueue_gmap() {
...
if (function_exists('cs_get_option')):
    wp_enqueue_style('event-term-fonts', event_term_fonts_url());
    add_action( 'wp_enqueue_scripts', 'event_term_custom_styles', 90 );
    add_action( 'wp_enqueue_scripts', 'event_term_enqueue_gmap',90 );
endif;
